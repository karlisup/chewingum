...
module.exports = function (opts, component, data) {
  ...
  if (opts.extensions.template === '.twig') { // Twig
    processTwig()
  } else if (opts.extensions.template === '.nunjucks') {
    processConsolidate()
  }
  ...
  function processConsolidate () {
    var templatingEngine = 'nunjucks';
    console.log(' - - - - Consolidate - - - - ')
    cons[templatingEngine](opts.location.src + ((component.demo) ? component.demo : component.tmpl.path), { user: 'tobi'  })
      .then(function (html) {
        console.log(html);
