...
function ProcessTemplate (opts) {
  this._opts = opts
  this._consolidateSupportedEngines = ['mustache', 'twig', 'nunjucks', 'handlebars']
}

ProcessTemplate.prototype.process = function (filePath, data, engine, callback) {
  if (supportedByConsolidate.call(this, engine)) {
    return processConsolidate(filePath, data, engine, callback)
  }
}

function supportedByConsolidate (templateEngine) {
  return this._consolidateSupportedEngines.indexOf(templateEngine) !== -1
}

function processConsolidate (filePath, data, engine, callback) {
  cons[engine](filePath, data)
  .then(function (returnedHtml) {
    console.log('1', returnedHtml)
